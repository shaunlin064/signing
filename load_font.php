...
EOD;
    ...
    function install_font_family ( $dompdf, $fontname, $normal, $bold = null, $italic = null, $bold_italic = null ) {
        ...
        if ( !is_readable($normal) ) throw new Exception("Unable to read '$normal'.");
        ...
        foreach ( $patterns as $type => $_patterns ) {
            if ( !isset($$type) || !is_readable($$type) ) {
                ...
                if ( is_null($$type) ) echo( "Unable to find $type face file.\n" );
            }
        }
        ...
        foreach ( $fonts as $var => $src ) {
            ...
            if ( !is_readable($src) ) throw new Exception("Requested font '$src' is not readable");

            $dest = $dompdf->getOptions()->get('fontDir') . '/' . basename($src);

            if ( !is_writeable(dirname($dest)) ) throw new Exception("Unable to write to destination '$dest'.");

            echo "Copying $src to $dest...\n";

            if ( !copy($src, $dest) ) throw new Exception("Unable to copy '$src' to '$dest'");
    ...
    if ( $_SERVER["argv"][1] === "system_fonts" ) {
        $fontMetrics = $dompdf->getFontMetrics();
        $files       = glob("/usr/share/fonts/truetype/*.ttf") + glob("/usr/share/fonts/truetype/*/*.ttf") + glob(
                "/usr/share/fonts/truetype/*/*/*.ttf"
            ) + glob("C:\\Windows\\fonts\\*.ttf") + glob("C:\\WinNT\\fonts\\*.ttf") + glob(
                "/mnt/c_drive/WINDOWS/Fonts/"
            );
        $fonts       = [];
        ...
        foreach ( $fonts as $family => $files ) {
            ...
            if ( !isset($files["normal"]) ) {
                ...
                install_font_family(
                    $dompdf, $family, @$files["normal"], @$files["bold"], @$files["italic"], @$files["bold_italic"]
                );
                echo "Done !\n";
            }
