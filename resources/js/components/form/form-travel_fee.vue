          ...
          <div class='form-group'>
            ...
            <select class='custom-select select2 form-control' id='form_pair_data_id' :disabled='form_action=="edit"'
              ...
              <option v-for='item in travel_grant_datas' :value='item.id'
                      :selected='form_submit_data[dom_id]["form_pair_data_id"]'
              >
                {{ item.apply_subject }}
              </option>
            </select>
...
import {apiGetDependList, apiGetPairData} from '../../src/apis/form'
  ...
  methods: {
    ...
    getTraveGrantData() {
      ...
      if (this.form_action == 'new') {
        ...
        parameter.id = this.form_submit_data[this.dom_id]['form_pair_data_id'];
        apiGetPairData(parameter)
            .then(function (response) {
              let result = response.data;
              if (result.status !== 1) {
                alert(result.message);
